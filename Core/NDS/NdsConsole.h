...
class NdsControlManager;
class VirtualFile;

class NdsConsole final : public IConsole
{
private:
	Emulator* _emu = nullptr;
	unique_ptr<LibretroCore> _core;
	unique_ptr<NdsControlManager> _controlManager;
...
public:
	NdsConsole(Emulator* emu);
	~NdsConsole();

	static vector<string> GetSupportedExtensions() { return { ".nds" }; }
	static vector<string> GetSupportedSignatures() { return { }; }

	// Load the melondsds libretro core
	bool LoadCore();

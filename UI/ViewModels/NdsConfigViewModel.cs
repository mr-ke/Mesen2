...
{
	public class NdsConfigViewModel : DisposableViewModel
	{
		[Reactive] public NdsConfig Config { get; set; }
		[Reactive] public NdsConfig OriginalConfig { get; set; }
		[Reactive] public NdsConfigTab SelectedTab { get; set; } = 0;

		public ReactiveCommand<Button, Unit> SetupPlayer { get; }

		public NdsConfigViewModel()
		{
			Config = ConfigManager.Config.Nds;
		...
		private async void OpenSetup(Button btn, int port)
		{
			...
			wnd.DataContext = new ControllerConfigViewModel(ControllerType.NdsController, cfg, Config.Controller, 0);
